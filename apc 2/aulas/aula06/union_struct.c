...
int main() {
  ...
  for(int i = 0; i < 100*4; i += 1){
    int media = round((pixels[i].rgba.b + pixels[i].rgba.g + pixels[i].rgba.r) /3.0);
    pixels[i].rgba.b = media;
    pixels[i].rgba.g = media;
    pixels[i].rgba.r = media;
  }

  // Copia o fundo da imagem original para imagem4
  for (int x = 0; x < 10; x++) {
      for (int y = 0; y < 10; y++) {
  ...
  for(int lin = 0; lin < 10; lin++) {
      for(int col = 0; col < 10; col++) {
          ...
          if(lin == col) {
              ...
              pchar4[(lin*10*4) + (col*4)+1] = 255; // Azul
          }
